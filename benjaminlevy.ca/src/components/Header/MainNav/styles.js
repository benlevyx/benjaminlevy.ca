...
export const MainNavItem = motion.custom(styled(Link)`
  ${tw`relative text-black border-b border-transparent hover:text-blue-400 ml-0 sm:ml-8 mt-3 sm:mt-0`};
  width: max-content;
  ...
  &:before {
    ${tw`absolute w-full bg-blue-400 h-px left-0 invisible`};
    content: '';
    bottom: -1px;
    transform: scaleX(0);
    transition: 0.2s;
...
export const ToogleMainNav = styled.button`
  ...
  span {
    ...
    &:nth-child(2) {
      ${tw`bg-blue-400 inline-block w-8 h-px`};
